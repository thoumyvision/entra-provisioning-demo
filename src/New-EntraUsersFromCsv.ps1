...
function Set-TapVaultSecret {
    ...
    # This function is only ever called from a code path already gated by the main loop's own
    # ShouldProcess, but it carries a real one of its own too - it genuinely writes to Key Vault,
    # so it earns the same guarantee every other mutation in this script has, not a decorative one.
    if ($PSCmdlet.ShouldProcess("$SecretName in $VaultName", 'Write Temporary Access Pass secret')) {
